...
import { UtensilsCrossed, ChevronRight, CheckCircle2 } from "lucide-react";
import { listPlanPatients } from "@/lib/endurance/planos";
import { loadModule, DeniedModule, ModuleHeader, EmptyCard } from "../module-kit";
import PatientSearch from "./patient-search";

/**
 * Planos alimentares — pacientes com plano + busca para abrir/criar. Nicho
 * nutrição (gate planos.manage).
 */
export default async function PlanosPage({
  ...
  const patients = session ? await listPlanPatients(session.org) : [];
  ...
  return (
    <div className="space-y-6">
      ...
      {patients.length === 0 ? (
        <EmptyCard>
          Nenhum plano ainda. Busque um paciente acima para montar o primeiro
          cardápio.
        </EmptyCard>
      ) : (
        <ul className="divide-y divide-slate-100 overflow-hidden rounded-2xl border border-slate-200 bg-white dark:divide-ink-800 dark:border-ink-700 dark:bg-ink-900">
          {patients.map((p) => (
            <li key={p.id}>
              <Link
                href={`/espaco/${slug}/m/planos_alimentares/${p.id}`}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-400">
                    {p.plans} plano{p.plans === 1 ? "" : "s"}
                    {p.lastAt &&
                      ` · atualizado em ${new Date(p.lastAt).toLocaleDateString("pt-BR")}`}
                  </p>
                </div>
                {p.hasActive && (
