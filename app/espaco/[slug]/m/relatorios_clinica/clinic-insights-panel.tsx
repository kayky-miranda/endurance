...
import { clinicInsightsAction } from "./relatorios-clinica-actions";
import type { Insight } from "@/lib/endurance/sales-insights";

const STYLE: Record<string, { icon: typeof Info; ring: string }> = {
  oportunidade: {
    icon: TrendingUp,
...
/**
 * Insights gerenciais da clínica (IA opcional). Refaz a análise sempre que o
 * período (`days`) muda, para acompanhar o filtro da página.
 */
export default function ClinicInsightsPanel({ days }: { days: number }) {
  ...
  async function load(d: number) {
    setLoading(true);
    const res = await clinicInsightsAction(d === 30 ? undefined : String(d));
    if (res.ok) {
    ...
    <div className="rounded-2xl border border-brand-500/30 bg-brand-500/5 p-5">
      ...
      {loading && insights.length === 0 ? (
        <p className="py-6 text-center text-sm text-slate-400">
          Analisando os indicadores da clínica…
        </p>
      ) : (
        <div className="grid gap-2.5 sm:grid-cols-2">
          {insights.map((it, i) => {
            ...
            return (
              ...
              >
                <div className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg ${st.ring}`}>
