...
import { searchStudentsAction, type StudentHit } from "./avaliacao-actions";

/** Busca de aluno para abrir/registrar avaliações físicas. */
export default function StudentSearch({ slug }: { slug: string }) {
  ...
  function onChange(v: string) {
    ...
    timer.current = setTimeout(async () => {
      const res = await searchStudentsAction(v);
      setHits(res);
      setOpen(true);
      setLoading(false);
    }, 250);
          ...
          {hits.length === 0 ? (
            ...
            hits.map((h) => (
              <li key={h.id}>
                <button
                  type="button"
                  onClick={() => router.push(`/espaco/${slug}/m/avaliacao/${h.id}`)}
