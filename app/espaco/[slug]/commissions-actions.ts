...
} from "@/lib/endurance/commissions";
...
export async function setProfessionalCouncilAction(
  userId: string,
  council: string,
): Promise<CommissionActionResult> {
  const gate = await requirePermission("settings.general");
  if (!gate.ok) return gate;
  const s = gate.session;
  const res = await setProfessionalCouncil(s.org, userId, council);
  if (!res.ok) return res;
  await logActivity(s, "professional.council", "Definiu o registro (conselho) de um profissional", userId);
  revalidatePath(`/espaco/${s.slug}`);
  return { ok: true };
}
